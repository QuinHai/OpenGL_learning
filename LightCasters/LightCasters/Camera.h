...
public:
	...
	void ProcessKeybroad(Camera_Movement direction, float deltaTime) {
		...
		if (direction == FORWARD) Position += fps_Front * velocity;
		if (direction == BACKWARD) Position -= fps_Front * velocity;
		if (direction == RIGHT) Position += fps_Right * velocity;
		if (direction == LEFT) Position -= fps_Right * velocity;
	...
	void ProcessMouseScroll(double yoffset) {//鼠标转轮
